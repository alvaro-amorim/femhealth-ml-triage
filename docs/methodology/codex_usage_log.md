...
| 4 | 2026-06-26 | Modelagem inicial e avaliação controlada | 37 testes passaram localmente | Candidatos treináveis em memória e métricas iniciais implementados | não disponível |
...
- Testes: 37 testes passaram localmente em `python -m pytest -q` e `pytest -q`.
- Resultado: Regressão Logística, Árvore de Decisão e KNN treinam em memória; métricas da classe maligna usam `pos_label=0` e probabilidade da classe 0 para ROC AUC.
